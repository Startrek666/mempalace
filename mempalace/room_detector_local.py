...
FOLDER_ROOM_MAP = {
    ...
    # Kept in the same map so ``detect_rooms_from_folders`` and
    # ``detect_rooms_from_files`` both pick them up without any additional
    # code paths. Only multi-character CJK keys are added — single-character
    # folder names ("前", "后") are too ambiguous and would false-match.
    "前端": "frontend",
    "前端开发": "frontend",
    "客户端": "frontend",
    "界面": "frontend",
